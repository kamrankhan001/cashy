            ...
            <form action="{{ route('admin.update-accounts', ['setting' => $setting->id]) }}" method="POST" novalidate>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="jazzcash_title" id="jazzcash_title" placeholder="Account Title" required
                            value="{{ $setting->jazzcash_account_title }}"
                            class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
        ...
        <div class="bg-white shadow-md rounded-lg p-6 overflow-x-auto">
            ...
            <form action="{{ route('admin.levels.update') }}" method="POST">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($levels as $level)
                                <tr>
                                    <td class="px-2 py-4 whitespace-nowrap text-sm">{{ $level->level_number }}</td>
                                    <td class="px-2 py-4 whitespace-nowrap">
                                        <input type="hidden" name="levels[{{ $loop->index }}][id]"
                                            value="{{ $level->id }}">
                                        <input type="number" name="levels[{{ $loop->index }}][members]"
                                            value="{{ old('levels.' . $loop->index . '.members', $level->members) }}"
                                            class="border rounded p-2 min-w-[5ch] w-full sm:w-auto">
                                    </td>
                                    <td class="px-2 py-4 whitespace-nowrap">
                                        <input type="number" name="levels[{{ $loop->index }}][task_income]"
                                            step="0.01"
                                            value="{{ old('levels.' . $loop->index . '.task_income', $level->task_income) }}"
                                            class="border rounded p-2 min-w-[5ch] w-full sm:w-auto">
                                    </td>
                                    <td class="px-2 py-4 whitespace-nowrap">
                                        <input type="number" name="levels[{{ $loop->index }}][referral_bonus]"
                                            step="0.01"
                                            value="{{ old('levels.' . $loop->index . '.referral_bonus', $level->referral_bonus) }}"
                                            class="border rounded p-2 min-w-[5ch] w-full sm:w-auto">
                                    </td>
                                    <td class="px-2 py-4 whitespace-nowrap">
                                        <input type="number" name="levels[{{ $loop->index }}][extra_coins]"
                                            value="{{ old('levels.' . $loop->index . '.extra_coins', $level->extra_coins) }}"
                                            class="border rounded p-2 min-w-[5ch] w-full sm:w-auto">
                                    </td>
                                </tr>
                            @endforeach
                ...
                <div class="mt-4 flex justify-end">
                    <button type="submit"
                        class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 w-full sm:w-auto">Update
                        Levels</button>
                </div>
            </form>
        </div>
